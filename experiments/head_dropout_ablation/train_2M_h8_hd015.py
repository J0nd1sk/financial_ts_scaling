#!/usr/bin/env python3
"""
Head Dropout Ablation: 2M h=8 with head_dropout=0.15
Purpose: Test effect of moderate head dropout on best 2M architecture
Architecture: d=64, L=32, h=8 (~2M params)
Baseline: head_dropout=0.0 achieved AUC 0.713
"""
...
HEAD_DROPOUT = 0.15  # <-- VARIABLE UNDER TEST
...
def main():
    ...
    print(f"Architecture: 2M (d={D_MODEL}, L={N_LAYERS}, h={N_HEADS})")
    print(f"Baseline (hd=0.0): AUC 0.713")
    print("=" * 70)
    ...
    print(f"Test AUC: {test_auc:.4f} (baseline: 0.713)")
    print(f"Test Accuracy: {test_accuracy:.4f}")
    print(f"Prediction spread: {pred_spread:.4f}")
    delta = test_auc - 0.713
    print(f"Delta vs baseline: {delta:+.4f} ({delta/0.713*100:+.1f}%)")
