#!/usr/bin/env python3
"""Reduce a500 tier features through iterative importance-based removal.

This experiment:
1. Trains baseline model on all 500 a500 features
...
Output:
    outputs/feature_curation/a500_reduction/
...
Usage:
    python experiments/feature_curation/reduce_a500.py --dry-run
    python experiments/feature_curation/reduce_a500.py
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Reduce a500 features")
    parser.add_argument("--dry-run", action="store_true", help="Show plan without executing")
    parser.add_argument("--max-rounds", type=int, default=25, help="Maximum reduction rounds")
    args = parser.parse_args()

    print("=" * 70)
    print("Feature Reduction: tier_a500 (500 features)")
    print("=" * 70)
    print()

    # Configuration - slightly more aggressive for larger tier
    ...
    # Output directory
    output_dir = PROJECT_ROOT / "outputs" / "feature_curation" / "a500_reduction"
    ...
    if args.dry_run:
        ...
        print("  - Train PatchTST on all 500 features")
        ...
        print("  - Processed data at data/processed/a500/")
        print("  - GPU/MPS available for training")
        print("  - ~4-6 hours runtime (larger feature set)")
        return 0
    ...
    print("1. Load processed data with a500 features")
