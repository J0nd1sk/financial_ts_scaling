#!/usr/bin/env python3
"""
Context Length Ablation for A200 Feature Tier.

Runs training at multiple context lengths (60d, 80d, 90d, 120d, 180d, 252d)
using the best architecture from a200 HPO to find optimal context for this tier.

IMPORTANT: Run this AFTER a200 HPO completes to use the optimized architecture.
The script will use the architecture defined in train_tier_ctx.py TIER_BEST_ARCH.

Usage:
    # Run full ablation (all 6 context lengths)
    python experiments/context_ablation_tiers/run_ctx_ablation_a200.py

    # Run single context length
    python experiments/context_ablation_tiers/run_ctx_ablation_a200.py --context 80

    # Dry run (show config only)
    python experiments/context_ablation_tiers/run_ctx_ablation_a200.py --dry-run
"""
import sys
import json
import argparse
...
def main():
    parser = argparse.ArgumentParser(
        description="Run context length ablation for a200 tier",
    ...
    print()
    print("NOTE: Ensure a200 HPO has completed and TIER_BEST_ARCH is updated")
    print("      in train_tier_ctx.py before running this ablation.")
    print()
