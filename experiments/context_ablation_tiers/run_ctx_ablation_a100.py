#!/usr/bin/env python3
"""
Context Length Ablation for A100 Feature Tier.

Runs training at multiple context lengths (60d, 80d, 90d, 120d, 180d, 252d)
using the best architecture from a100 HPO to find optimal context for this tier.

Usage:
    # Run full ablation (all 6 context lengths)
    python experiments/context_ablation_tiers/run_ctx_ablation_a100.py

    # Run single context length
    python experiments/context_ablation_tiers/run_ctx_ablation_a100.py --context 80

    # Dry run (show config only)
    python experiments/context_ablation_tiers/run_ctx_ablation_a100.py --dry-run
"""
import sys
import json
import argparse
...
def main():
    parser = argparse.ArgumentParser(
        description="Run context length ablation for a100 tier",
