#!/usr/bin/env python3
"""
PHASE6A Experiment: 2B parameters, threshold_1pct task - RESUME SCRIPT
Type: HPO (Hyperparameter Optimization) with Architectural Search

This script RESUMES from saved trials:
- Loads trials 0-10 from outputs/hpo/phase6a_2B_h1_threshold_1pct/trials/
- Injects them into the Optuna study
- Skips arch_idx=52 (d=1024, L=256 which caused memory issues)
- Continues from trial 11 to completion

Generated: 2026-01-03
Updated: 2026-01-03 (load 11 trials, add diversity forcing)
"""
import json
import sys
import time
...
EXPERIMENT = "phase6a_2B_h1_threshold_1pct"
PHASE = "phase6a"
BUDGET = "2B"
TASK = "threshold_1pct"
HORIZON = 1
...
# HPO settings - resuming from trial 11
N_TRIALS_TOTAL = 50
N_TRIALS_LOADED = 11  # Trials 0-10 will be loaded
N_TRIALS_REMAINING = N_TRIALS_TOTAL - N_TRIALS_LOADED  # 39 more trials
...
# Architecture to skip (caused memory issues: d=1024, L=256)
SKIP_ARCH_IDX = 52
...
print(f"⚠️ Skipping arch_idx={SKIP_ARCH_IDX} (d=1024, L=256 - memory issues)")
...
# ============================================================
# MODIFIED OBJECTIVE THAT SKIPS PROBLEMATIC ARCHITECTURE
# ============================================================

def create_objective_with_skip(base_objective, skip_arch_idx):
    """Wrap objective to skip problematic architecture."""
    def objective(trial):
        # For non-forced trials, check if we're about to use the bad architecture
        # We can't prevent Optuna from sampling it, but we can return a bad value
        arch_idx = trial.suggest_int("arch_idx", 0, len(ARCHITECTURES) - 1)

        if arch_idx == skip_arch_idx:
            print(f"  ⚠️ Skipping arch_idx={arch_idx} (known to cause memory issues)")
            # Return a high loss to discourage this architecture
            return float('inf')

        # Otherwise run normally - but we need to re-sample since we already consumed arch_idx
        # Actually, we need a different approach - let's modify the architectures list instead
...
if __name__ == "__main__":
    ...
    # Filter architectures to exclude problematic one
    filtered_architectures = [
        arch for i, arch in enumerate(ARCHITECTURES)
        if i != SKIP_ARCH_IDX
    ]
    print(f"✓ Filtered architectures: {len(filtered_architectures)} (excluded idx={SKIP_ARCH_IDX})")

    # Create architectural objective with filtered architectures
    objective = create_architectural_objective(
        config_path=str(PROJECT_ROOT / CONFIG_PATH),
        budget=BUDGET,
        architectures=filtered_architectures,  # Use filtered list
    ...
    output_path = save_best_params(
        ...
        budget=BUDGET,
        output_dir=OUTPUT_DIR,
        architectures=ARCHITECTURES,  # Use full list for reference
    )
    ...
    print(f"  Total trials: {len(study.trials)} ({N_TRIALS_LOADED} loaded + {N_TRIALS_REMAINING} new)")
