#!/usr/bin/env python3
"""
0.5% Threshold with HIGH-based Targets: 2M Narrow-Deep
Purpose: Test if best 2M architecture performs well with correct targets
Architecture: d=64, L=32, h=2 (~2M params) - best from HPO for h3

This was the "BEST OVERALL" from Phase 6A HPO (though trained on CLOSE).
Testing if narrow-deep architecture works better than wide-shallow at smaller scale.
"""
...
EXPERIMENT_NAME = "2M_narrow_deep_threshold_05pct_HIGH"
TASK = "threshold_0.5pct"
HORIZON = 1

# Architecture: 2M narrow-deep (best from HPO h3)
D_MODEL = 64
N_LAYERS = 32
N_HEADS = 2
D_FF = 256  # 4 * d_model
CONTEXT_LENGTH = 80
DROPOUT = 0.5  # Same as 20M experiments for comparison

# Training (same as 20M for fair comparison)
LEARNING_RATE = 1e-4
...
def main():
    ...
    print("Architecture: 2M narrow-deep (d=64, L=32, h=2)")
    print("=" * 70)
    ...
    # Estimate parameters
    # Rough estimate: embedding + L*(attention + ffn) + head
    # attention per layer: 4 * d^2 (Q,K,V,O projections)
    # ffn per layer: 2 * d * d_ff
    ...
    # Compare with 20M results
    print(f"\n{'=' * 70}")
    print("COMPARISON WITH 20M MODELS")
    print(f"{'=' * 70}")
    print(f"{'Model':<20} {'Test AUC':<12} {'Test Acc':<12} {'Test F1':<12}")
    print("-" * 56)
    print(f"{'2M narrow-deep':<20} {test_auc:<12.4f} {test_accuracy:<12.4f} {test_f1:<12.4f}")
    print(f"{'20M h=4 (best)':<20} {'0.6900':<12} {'0.6464':<12} {'0.5077':<12}")
    print(f"{'20M h=8':<20} {'0.7000':<12} {'0.6077':<12} {'0.4034':<12}")
