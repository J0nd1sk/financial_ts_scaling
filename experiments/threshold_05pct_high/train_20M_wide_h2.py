#!/usr/bin/env python3
"""
0.5% Threshold with HIGH-based Targets: 20M_wide h=2
Purpose: Compare n_heads=2 vs n_heads=4/8 with correct HIGH-based targets
Architecture: d=512, L=6, h=2 (20M_wide variant with 2 heads)
Target: max(HIGH[t+1:t+horizon]) >= CLOSE[t] * 1.005

Previous (invalid) results showed h=2 as best, but that was train/eval mismatch.
This tests if h=2 is still optimal with correct HIGH-based targets.
"""
...
EXPERIMENT_NAME = "20M_wide_h2_threshold_05pct_HIGH"
TASK = "threshold_0.5pct"
HORIZON = 1

# Architecture: 20M_wide with h=2
D_MODEL = 512
N_LAYERS = 6
N_HEADS = 2  # Testing if h=2 is still optimal
D_FF = 2048
CONTEXT_LENGTH = 80
DROPOUT = 0.5
