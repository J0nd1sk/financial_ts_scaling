#!/usr/bin/env python3
"""
Foundation Investigation: Lag-Llama H3, Close-only (FD-02a)
...
Success criteria: AUC >= 0.65 (5% improvement)
"""
import sys
import json
import time
...
EXPERIMENT_NAME = "lagllama_h3_close"
HORIZON = 3
...
def create_close_only_dataset(df, split_indices, context_length, horizon, threshold=0.01):
    ...
    def create_split(indices):
        ...
        for i in indices:
            ...
            if end_idx + horizon > len(close_prices):
                continue

            # Input: Close prices only, shape (context_length, 1)
            x = close_prices[i:end_idx].reshape(-1, 1)
...
def main():
    print("=" * 70)
    print(f"FOUNDATION INVESTIGATION: Lag-Llama / H{HORIZON} / Close-only (FD-02a)")
    print("=" * 70)
    ...
    # Create datasets
    print("\nCreating datasets (Close-only univariate)...")
    train_X, train_y, val_X, val_y = create_close_only_dataset(
    ...
    # Compare to baseline
    baseline_auc = 0.622  # PatchTST 200M H3
    if val_metrics['auc']:
        improvement = (val_metrics['auc'] - baseline_auc) / baseline_auc * 100
        print(f"\nVs PatchTST baseline (0.622): {improvement:+.1f}%")
        if val_metrics['auc'] >= 0.65:
            print("SUCCESS: Exceeds 5% improvement target!")
        else:
            print(f"Need {0.65 - val_metrics['auc']:.4f} more AUC for 5% improvement")

    # Save results
    results = {
        "experiment": EXPERIMENT_NAME,
        "experiment_id": "FD-02a",
        "model": "Lag-Llama",
