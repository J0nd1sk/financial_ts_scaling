#!/usr/bin/env python3
"""
Phase 6C Stage 1: a50 Baseline - 20M parameters, horizon=1

Tests feature scaling hypothesis: Does 55 features improve over 25 at same architecture?

Architecture (inherited from Phase 6A 20M):
...
Data:
    Features: 55 (5 OHLCV + 50 indicators from tier_a50)
    Splits: SimpleSplitter (Train through 2022, Val 2023-2024, Test 2025+)
"""
import sys
import json
import time
...
EXPERIMENT_NAME = "s1_02_20m_h1_a50"
BUDGET = "20M"
HORIZON = 1
...
NUM_FEATURES = 50  # Model sees 50 indicators (OHLCV used for targets)
...
def main():
    print("=" * 70)
    print(f"PHASE 6C STAGE 1: {BUDGET} / horizon={HORIZON} / features=55 (a50)")
    print("=" * 70)
    ...
    # Comparison baseline (Phase 6A a20 results)
    print(f"\n" + "-" * 70)
    print("COMPARISON (Phase 6A a20 baseline):")
    print(f"  Phase 6A 20M H1 AUC: 0.715")
    print(f"  Phase 6C 20M H1 AUC: {val_metrics['auc']:.3f}" if val_metrics['auc'] else "  Phase 6C: N/A")
    if val_metrics['auc']:
        delta = (val_metrics['auc'] - 0.715) / 0.715 * 100
