#!/usr/bin/env python3
"""
Phase 6C Stage 2: Training Parameter Experiment - 200M, weight_decay=1e-3

Tests if stronger L2 regularization helps fix 200M regression with 55 features.

Training Change:
    weight_decay=1e-3 (from 0) - Strong L2 regularization

Baseline (S1 200M H1 a50): AUC=0.699 (REGRESSED from 0.718 a20)
"""
import sys
import json
import time
...
EXPERIMENT_NAME = "s2_train_200m_h1_wd1e3"
BUDGET = "200M"
HORIZON = 1
...
# Hyperparameters - MODIFIED: strong weight decay
LEARNING_RATE = 1e-4
DROPOUT = 0.5
CONTEXT_LENGTH = 80
WEIGHT_DECAY = 1e-3   # NEW: Strong L2 regularization
...
# Baseline for comparison (S1 200M result - regressed)
BASELINE_AUC = 0.699
...
def main():
    print("=" * 70)
    print(f"PHASE 6C STAGE 2: {BUDGET} / horizon={HORIZON} / TRAIN: wd={WEIGHT_DECAY}")
    print("=" * 70)
    ...
    # Trainer with RevIN and weight_decay
    trainer = Trainer(
    ...
    # Comparison baseline
    print(f"\n" + "-" * 70)
    print(f"COMPARISON (S1 {BUDGET} H1 a50 baseline - REGRESSED):")
    print(f"  S1 Baseline AUC: {BASELINE_AUC}")
    print(f"  Phase 6A a20 AUC: 0.718")
    if val_metrics['auc']:
        print(f"  Current AUC: {val_metrics['auc']:.3f}")
        delta_s1 = (val_metrics['auc'] - BASELINE_AUC) / BASELINE_AUC * 100
        delta_6a = (val_metrics['auc'] - 0.718) / 0.718 * 100
        print(f"  Delta vs S1: {delta_s1:+.1f}%")
        print(f"  Delta vs 6A: {delta_6a:+.1f}%")
    print("-" * 70)
