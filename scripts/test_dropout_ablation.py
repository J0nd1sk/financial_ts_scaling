...
Tests effect of higher dropout on PatchTST and MLP models.
One variable at a time: Dropout varies, LR fixed at baseline (1e-3).

Configurations:
- B1: PatchTST @ LR=1e-3, dropout=0.4
- B2: PatchTST @ LR=1e-3, dropout=0.5
- B3: MLP @ LR=1e-3, dropout=0.4
- B4: MLP @ LR=1e-3, dropout=0.5
...
STRIDE = 8
BATCH_SIZE = 128
EPOCHS = 50  # Standard epochs for baseline LR
BASELINE_LR = 1e-3
...
CONFIGS = [
    {"name": "PatchTST_d0.4", "model": "PatchTST", "lr": BASELINE_LR, "dropout": 0.4},
    {"name": "PatchTST_d0.5", "model": "PatchTST", "lr": BASELINE_LR, "dropout": 0.5},
    {"name": "MLP_d0.4", "model": "MLP", "lr": BASELINE_LR, "dropout": 0.4},
    {"name": "MLP_d0.5", "model": "MLP", "lr": BASELINE_LR, "dropout": 0.5},
]
...
def main():
    print("=" * 70)
    print("DROPOUT ABLATION EXPERIMENT")
    print("Testing: Higher dropout (0.4, 0.5) vs baseline (0.2)")
    print("Fixed: LR = 1e-3 (baseline)")
    print("=" * 70)
    ...
    for config in CONFIGS:
        ...
        csv_path = OUTPUT_DIR / "dropout_ablation_results.csv"
        pd.DataFrame(results).to_csv(csv_path, index=False)

    # Summary
    print(f"\n{'=' * 70}")
    print("SUMMARY: Dropout Ablation Experiment")
    print(f"{'=' * 70}")
    print(f"{'Config':<20} {'Model':<10} {'Dropout':<10} {'Best AUC':<10} {'Final AUC':<10} {'Gap':<8}")
    print("-" * 68)

    for r in results:
        gap = r["best_val_auc"] - r["final_val_auc"]
        gap_str = f"{gap:+.4f}" if gap != 0 else "0.0000"
        print(f"{r['config_name']:<20} {r['model_type']:<10} {r['dropout']:<10.1f} "
    ...
    output = {
        ...
        "description": "Testing higher dropout (0.4, 0.5) vs baseline (0.2)",
        "fixed_params": {
    ...
    json_path = OUTPUT_DIR / "dropout_ablation_results.json"
