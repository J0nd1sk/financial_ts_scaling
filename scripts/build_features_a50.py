#!/usr/bin/env python
"""Build tier a50 features for a given ticker.

Usage:
    ./venv/bin/python scripts/build_features_a50.py \\
        --ticker SPY \\
        --raw-path data/raw/SPY.parquet \\
        --vix-path data/raw/VIX.parquet \\
        --output-path data/processed/v1/SPY_dataset_a50.parquet
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build tier a50 features")
    ...
    # Build features
    print("Building tier a50 features...")
    feature_df = tier_a50.build_feature_dataframe(raw_df, vix_df)

    # Report results
    print(f"\nFeature DataFrame:")
    print(f"  Rows: {len(feature_df)}")
    print(f"  Columns: {len(feature_df.columns)} (Date + {len(tier_a50.FEATURE_LIST)} features)")
    ...
    expected_features = len(tier_a50.FEATURE_LIST)
    ...
    # Print feature list
    print("\nFeatures (a50):")
    for i, feat in enumerate(tier_a50.FEATURE_LIST, 1):
        print(f"  {i:2d}. {feat}")
