#!/usr/bin/env python3
"""
Generate Phase 6A training scripts with corrected infrastructure.

Generates 12 scripts: 3 scales (2M, 20M, 200M) × 4 horizons (1, 2, 3, 5)

All scripts use:
- SimpleSplitter (442 val samples, not ChunkSplitter's 19)
- RevIN normalization (no global z-score)
- Dropout=0.5, LR=1e-4, Context=80 (ablation-validated)
- Full metrics: AUC, accuracy, precision, recall, pred_range
- Option A architectures (PatchTST-standard scaling)
"""

from pathlib import Path

# Architecture configurations (Option A: PatchTST-standard)
ARCHITECTURES = {
    "2M": {"d_model": 64, "n_layers": 4, "n_heads": 4, "d_ff": 256},
    "20M": {"d_model": 128, "n_layers": 6, "n_heads": 8, "d_ff": 512},
    "200M": {"d_model": 256, "n_layers": 8, "n_heads": 8, "d_ff": 1024},
}
...
"""
Phase 6A Training: {budget} parameters, horizon={horizon}
...
EXPERIMENT_NAME = "phase6a_{budget_lower}_h{horizon}"
BUDGET = "{budget}"
HORIZON = {horizon}
...
def generate_scripts():
    """Generate all 12 Phase 6A training scripts."""
    output_dir = Path(__file__).parent.parent / "experiments" / "phase6a_final"
    output_dir.mkdir(parents=True, exist_ok=True)

    generated = []
    for budget, arch in ARCHITECTURES.items():
        for horizon in HORIZONS:
            script_name = f"train_{budget}_h{horizon}.py"
