#!/usr/bin/env python
"""Build tier a500 features for a given ticker.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build tier a500 features")
    ...
    # Build features
    print("Building tier a500 features...")
    feature_df = tier_a500.build_feature_dataframe(raw_df, vix_df)

    # Report results
    print(f"\nFeature DataFrame:")
    print(f"  Rows: {len(feature_df)}")
    print(f"  Columns: {len(feature_df.columns)} (Date + {len(tier_a500.FEATURE_LIST)} features)")
    ...
    expected_features = len(tier_a500.FEATURE_LIST)
    ...
    print(f"\nFeatures summary:")
    print(f"  a200 features (1-206): {tier_a500.FEATURE_LIST[0]} ... {tier_a500.FEATURE_LIST[205]}")
    print(f"  a500 new features (207-300): {tier_a500.FEATURE_LIST[206]} ... {tier_a500.FEATURE_LIST[-1]}")
    print(f"  Total: {len(tier_a500.FEATURE_LIST)} features")
