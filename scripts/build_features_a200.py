#!/usr/bin/env python
"""Build tier a200 features for a given ticker.

Usage:
    ./venv/bin/python scripts/build_features_a200.py \
        --ticker SPY \
        --raw-path data/raw/SPY.parquet \
        --vix-path data/raw/VIX.parquet \
        --output-path data/processed/v1/SPY_features_a200.parquet
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build tier a200 features")
    ...
    # Build features
    print("Building tier a200 features...")
    feature_df = tier_a200.build_feature_dataframe(raw_df, vix_df)

    # Report results
    print(f"\nFeature DataFrame:")
    print(f"  Rows: {len(feature_df)}")
    print(f"  Columns: {len(feature_df.columns)} (Date + {len(tier_a200.FEATURE_LIST)} features)")
    ...
    expected_features = len(tier_a200.FEATURE_LIST)
    ...
    print("\nFeatures (a200) - first 100 from a100:")
    for i, feat in enumerate(tier_a200.FEATURE_LIST[:100], 1):
        if i <= 10 or i > 90:
            print(f"  {i:3d}. {feat}")
        elif i == 11:
            print(f"  ... (omitting {i} to 90)")

    print("\nFeatures (a200) - new 106 from a200:")
    for i, feat in enumerate(tier_a200.FEATURE_LIST[100:], 101):
        if i <= 110 or i > 196:
            print(f"  {i:3d}. {feat}")
        elif i == 111:
            print(f"  ... (omitting {i} to 196)")
