#!/usr/bin/env python3
"""
LR Ablation Experiment
Tests effect of lower learning rates on PatchTST and MLP models.
One variable at a time: LR varies, dropout fixed at baseline (0.2).

Configurations:
- A1: PatchTST @ LR=1e-5, dropout=0.2
- A2: PatchTST @ LR=1e-6, dropout=0.2
- A3: MLP @ LR=1e-5, dropout=0.2
- A4: MLP @ LR=1e-6, dropout=0.2
...
THRESHOLD = 0.01  # 1% threshold task

# Configurations to test
CONFIGS = [
    {"name": "PatchTST_lr1e-5", "model": "PatchTST", "lr": 1e-5, "dropout": BASELINE_DROPOUT},
    {"name": "PatchTST_lr1e-6", "model": "PatchTST", "lr": 1e-6, "dropout": BASELINE_DROPOUT},
    {"name": "MLP_lr1e-5", "model": "MLP", "lr": 1e-5, "dropout": BASELINE_DROPOUT},
    {"name": "MLP_lr1e-6", "model": "MLP", "lr": 1e-6, "dropout": BASELINE_DROPOUT},
]
...
def train_model(
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_X, batch_y in train_loader:
            ...
            # Handle PatchTST output shape (batch, 1) vs MLP (batch,)
            if preds.dim() == 2:
...
def main():
    print("=" * 70)
    print("LR ABLATION EXPERIMENT")
    print("Testing: Lower learning rates (1e-5, 1e-6) vs baseline (1e-3)")
    print("Fixed: Dropout = 0.2 (baseline)")
    print("=" * 70)
    ...
    for config in CONFIGS:
        ...
        csv_path = OUTPUT_DIR / "lr_ablation_results.csv"
        pd.DataFrame(results).to_csv(csv_path, index=False)

    # Summary
    print(f"\n{'=' * 70}")
    print("SUMMARY: LR Ablation Experiment")
    print(f"{'=' * 70}")
    print(f"{'Config':<20} {'Model':<10} {'LR':<10} {'Best AUC':<10} {'Final AUC':<10} {'Gap':<8}")
    print("-" * 68)

    for r in results:
        gap = r["best_val_auc"] - r["final_val_auc"]
        gap_str = f"{gap:+.4f}" if gap != 0 else "0.0000"
        print(f"{r['config_name']:<20} {r['model_type']:<10} {r['learning_rate']:<10.0e} "
              f"{r['best_val_auc']:.4f}     {r['final_val_auc']:.4f}     {gap_str}")
    ...
    for r in results:
        if r["model_type"] == "PatchTST":
            diff = r["best_val_auc"] - patchtst_baseline
            print(f"{r['config_name']}: Best AUC {r['best_val_auc']:.4f} "
                  f"({diff:+.4f} vs baseline)")
        else:
            diff_best = r["best_val_auc"] - mlp_best_baseline
            diff_final = r["final_val_auc"] - mlp_final_baseline
            print(f"{r['config_name']}: Best AUC {r['best_val_auc']:.4f} ({diff_best:+.4f}), "
                  f"Final AUC {r['final_val_auc']:.4f} ({diff_final:+.4f})")

    # Save final results
    output = {
        "experiment": "lr_ablation",
        "description": "Testing lower learning rates (1e-5, 1e-6) vs baseline (1e-3)",
        "fixed_params": {
    ...
    json_path = OUTPUT_DIR / "lr_ablation_results.json"
